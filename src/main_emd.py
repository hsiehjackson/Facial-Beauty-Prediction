...
def main():
    if args.MODE == 'train':
        for CV in range(1,6):
            ...
            file_name = 'cv{}_emd_{}'.format(CV,args.use_model)
...
def train(CV,train_csv_file,val_csv_file,file_name):
    ...
    for epoch in range(0, args.EPOCHS):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            if batch_idx > 0:                
                print('\rCV{} Epoch: {}/{} | EMD: {:.4f} | PC: {:.4f} | MAE: {:.4f} | RMSE: {:.4f} | [{}/{} ({:.0f}%)] | Time: {}  '.format(CV ,epoch + 1, args.EPOCHS, 
                loss.data[0], PC, MAE, RMSE, batch_idx*args.train_batch_size, len(train_loader.dataset),
        ...
        train_losses.append(emd_loss(target, predicted))
        ...
        val_losses.append(emd_loss(target, predicted))
        ...
        print('\ntrain EMD %.4f | train PC %.4f | train MAE %.4f | train RMSE: %.4f' % (train_losses[-1], train_pc[-1], train_mae[-1], train_rmse[-1]))
        print('valid EMD %.4f | valid PC %.4f | valid MAE %.4f | valid RMSE: %.4f' % (val_losses[-1], val_pc[-1], val_mae[-1], val_rmse[-1]))

        # Use early stopping to monitor training
        if val_losses[-1] < best_val_loss:
            best_val_loss = val_losses[-1]
            # save model weights if val loss decreases
            torch.save(model.state_dict(), os.path.join(save_path, 'EMD-%f.pkl' % (best_val_loss)))
            print ('Save Improved Model(EMD_loss = %.6f)...' % (best_val_loss))
            # reset stop_count
        if save_fig and (epoch+1) % 100 == 0:
            epochs = range(1, epoch + 2)
            plt.plot(epochs, train_losses, 'b-', label='train emd')
            plt.plot(epochs, val_losses, 'g-', label='val emd')
...
def test(CV,val_csv_file):
    ...
    for batch_idx, data in enumerate(test_loader):
        ...
        if args.plot:
            ...
            print('beauty score: {:.2f}%{:.2f}({:.2f}%{:.2f})'.format(output_score,output_score_std,target_score,target_score_std))
            plt.imshow(img)
            plt.show()
