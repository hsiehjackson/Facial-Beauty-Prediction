...
def main():
    if args.MODE == 'train':
        for CV in range(1,6):
            ...
            file_name = 'cv{}_bce_{}'.format(CV,args.use_model)
...
def train(CV,train_csv_file,val_csv_file,file_name):
    ...
    for epoch in range(0, args.EPOCHS):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            if batch_idx > 0:                
                print('\rCV{} Epoch: {}/{} | CE: {:.4f} | PC: {:.4f} | MAE: {:.4f} | RMSE: {:.4f} | [{}/{} ({:.0f}%)] | Time: {}  '.format(CV ,epoch + 1, args.EPOCHS, 
        ...
        print('\ntrain CE %.4f | train PC %.4f | train MAE %.4f | train RMSE: %.4f' % (train_losses[-1], train_pc[-1], train_mae[-1], train_rmse[-1]))
        print('valid CE %.4f | valid PC %.4f | valid MAE %.4f | valid RMSE: %.4f' % (val_losses[-1], val_pc[-1], val_mae[-1], val_rmse[-1]))

        # Use early stopping to monitor training
        if val_losses[-1] < best_val_loss:
            best_val_loss = val_losses[-1]
            # save model weights if val loss decreases
            torch.save(model.state_dict(), os.path.join(save_path, 'BCE-%f.pkl' % (best_val_loss)))
...
def test(CV,val_csv_file):
    ...
    for batch_idx, data in enumerate(test_loader):
        ...
        outprobs = metrics(outputs)
        predicted_mean, target_mean = 0.0, 0.0
        predicted_std, target_std = 0.0, 0.0
        for i in range(5):
            predicted_mean += i * outprobs[:,i].cpu()
            target_mean += i * labels[:,i].cpu()
        for i in range(5):
            predicted_std += outprobs[:,i].cpu() * (i - predicted_mean) ** 2
    ...
    print('CE LOSS: {:.4f}'.format(loss_fn(predicted_prob, target_prob)))
